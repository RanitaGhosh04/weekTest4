...
const EmployeeList = ({ employees, onAddToTeam }) => {
    ...
    const handleAddToTeam = (employee) => {
      ...
      setFilteredEmployees(filteredEmployees.filter((emp) => emp.id !== employee.id));
    };
        ...
        <ul>
          {filteredEmployees.map((employee) => (
            <li key={employee.id}>
              ...
              <button className=' float-end' onClick={() => handleAddToTeam(employee)} disabled={!filteredEmployees.some((emp) => emp.id === employee.id)}>
                ADD
              </button>
            </li>
          ))}
  ...
  const TeamMembers = ({ team, averageAge, onSortByAge, onRemoveFromTeam }) => {
    const handleSortByAge = () => {
      onSortByAge();
    };
  
    return (
      <div className="team-members bg-gray-200 p-4 rounded shadow-md">
        <h2>Team Members</h2>
        ...
        <p className="average-age">Average Age: {averageAge.toFixed(2)}</p>
      </div>
    );
  };
  ...
  const App = () => {
    ...
    const calculateAverageAge = () => {
      ...
      setAverageAge(totalAge / teamMembers.length || 0);
    };
  
    const sortByAge = () => {
      setTeamMembers([...teamMembers].sort((a, b) => a.age - b.age));
    };
    ...
    return (
      <div className="App">
        ...
        <div className="container flex flex-row">
          ...
          <TeamMembers team={teamMembers} averageAge={averageAge} onSortByAge={sortByAge} onRemoveFromTeam={removeFromTeam} />
        </div>
      </div>
    );
  };
